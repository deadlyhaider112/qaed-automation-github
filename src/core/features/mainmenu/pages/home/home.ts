...
import {UpcomingTrainingsApplyAlertComponent} from "@features/courses/components/upcoming-trainings-apply-alert/upcoming-training-alert";
...
export class CoreMainMenuHomePage implements OnInit {
    ...
    enrolledCategories: CoreCategoryData[] = [];
    upcomingCategories: CoreCategoryData[] = [];
    ...
    constructor(private appLauncher: AppLauncher, private market: Market, public modalController: ModalController) {
        ...
        this.fetchCategories().finally(() => {
            for (let i = 0; i < this.categories.length; i++) {
                if (this.categories[i].is_upcoming == '1') {
                    ...
                    this.enrolledCategories.push(this.categories[i]);
                }
            }
    ...
    async presentSuccessDialog() {
        const modal = await this.modalController.create({
            component: UpcomingTrainingsApplyAlertComponent,
    ...
    appLaunch(id: any, packageName: string) {
        if (packageName === 'aeo_package') {
            CoreNavigator.navigate('launch');

            return;
        }

        this.launch(packageName);
    }

    protected launch(packageName: string) {
        ...
        this.appLauncher.canLaunch(options)
            .then((canLaunch: boolean) => {
                if (canLaunch) {
                    this.appLauncher.launch(options).then(value => {
                        CoreDomUtils.showToast(value);
                    });
                }
            })
            .catch((error: any) => {
