...
import {VideoModalPageComponent} from '@components/video-modal/video-modal-page';
...
export class CoreLoginCredentialsPage implements OnInit, OnDestroy {
    ...
    introVideo: string | undefined;

    videoOptions: ModalOptions = {
        component: VideoModalPageComponent,
        componentProps: {},
    };
    ...
    async introductoryVideo(): Promise<void> {
        this.videoOptions.componentProps!.introductoryVideo = this.introVideo;
        CoreDomUtils.openModal(this.videoOptions);
    }
    ...
    protected treatSiteConfig(): void {
        if (this.siteConfig) {
            ...
            this.introVideo = this.siteConfig.introductoryvideo;
    ...
    async login(e?: Event): Promise<void> {
        ...
        try {
            ...
            // await CoreNavigator.navigateToSitePath('courses/categories/root');
        } catch (error) {
    ...
    openFaqs(): void {
        CoreNavigator.navigate('/login/faqs');
    }
