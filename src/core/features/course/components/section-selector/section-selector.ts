...
import { CoreCourseSection, CoreCourseSectionWithStatus } from '@features/course/services/course-helper';
...
export class CoreCourseSectionSelectorComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.sections = this.sections?.filter(function (el) { return el.name != "General"; });
        this.sections = this.sections?.filter(function (el) { return el.name != "All sections"; });
        this.sections = this.sections?.filter(function (el) { return el.name != "List of Units"; });

        this.sec = this.sections;

        this.sec = arrayToTree(this.sec, { nestedIds: false });
    ...
    expandItem(item: { expanded: boolean; }): void {
        ...
        if (item.expanded) {
            ...
            this.sec.map(listItem => {
                if (listItem.children.length > 0) {

                    if (item == listItem.data) {

                        listItem.data.expanded = !listItem.data.expanded;

                    } else {
                        listItem.data.expanded = false;
                    }
                }
                return listItem.children.data;
